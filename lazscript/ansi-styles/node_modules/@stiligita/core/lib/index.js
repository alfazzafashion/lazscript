...
var _domElements = require('@stiligita/dom-elements');
...
function styled(tag, props) {
  return (strings, ...args) => {
    ...
    return _dom.render.createComponent(strings, args, tag, props);
  };
}

// Add static `use` to allow setting plugins from `styled`
// prevents having to expose the entire render instance
styled.use = plugin => _dom.render.use(plugin);
styled.before = plugin => _dom.render.before(plugin);

// Add literals for each DOM element
_domElements.domElements.forEach(tag => {
  styled[tag] = (strings, ...args) => _dom.render.createComponent(strings, args, tag);
  // Future API
